...
**Alert rules (11):** `HighErrorRate` (>5%, 2m), `ModelNotLoaded` (`spendsense_model_loaded == 0 or up{job="spendsense_backend"} == 0`, 1m — the `or up==0` clause is essential because `spendsense_model_loaded` is pulled from the backend, so when the backend goes down the metric becomes stale/absent and `==0` alone never matches), `HighPredictionLatency` (p95>500ms, 5m), `LowTestF1` (<0.70, instant), `LowValF1` (<0.65, instant), `TrainingDurationHigh` (>2h, instant), `DataDriftDetected` (instant), `IngestFailed` (instant), `FeedbackLoopDead` (no feedback in 24h, fires after 48h), `TailLatencySpike` (p99>1s, 5m), `FrequentModelSwitch` (>3/h, instant)

**Grafana panels (8, auto-provisioned):** Request Rate (req/s), Model Loaded (uses `sum(spendsense_model_loaded * up{job="spendsense_backend"}) or vector(0)` so it goes red within ~10 s of the backend stopping, instead of staying green for the 5-minute staleness window), Predictions by Category, Total Requests, Latency Percentiles (P50/P95/P99), Airflow Drift Flag, Alerts Fired by Name (pie — `count_over_time(ALERTS{alertstate="firing"}[$__range])` summed by `alertname`), Recent Email Alerts (table of currently-firing alerts). Note: Grafana internal port is 3000, host port is 3001 — Docker inter-service config uses `grafana:3000`.

...
Three jobs (total ~47 min):

- **Job 1 (test, ~35s)**: flake8 + pytest on every push to any branch
- **Job 2 (ml-pipeline, ~26 min)**: main-branch only
  ...
  4. Starts Airflow, triggers `spendsense_ingestion_pipeline` DAG (drift detection → `combine_data` merges 90%+10%+feedback)
  5. **DVC Run 2** fine-tunes with `FINETUNE_MODEL_PATH=models/run1_model.pt` on merged data
- **Job 3 (app, ~21 min)**: main-branch only — downloads Job 2 artifacts (model, vocab, label_encoder, mlruns), smoke-tests FastAPI + Streamlit via Docker

**Largest CI overhead:** Job 3 downloads 276 MB of numpy arrays it never uses. Scoping artifact upload to only `models/latest_model.pt`, vocab, label_encoder, and mlruns would save ~15–18 min (see `docs/overhead.md`).
...
Three pages:
- **Home** (`Home.py`): single prediction, 6 example buttons that pre-fill the input, confidence bar chart, post-prediction feedback form (calls `POST /feedback`), sidebar model-switching dropdown (calls `POST /models/switch`)
- **Batch Predict** (`pages/1_Batch_Predict.py`): three tabs — CSV upload, paste descriptions, HDFC bank statement XLS upload (auto-detects header row, filters withdrawal transactions). Results table, Altair donut chart, CSV download button
- **Pipeline Status** (`pages/2_Pipeline_Status.py`): health grid for all 7 services, live Prometheus metric counters, DVC DAG diagram via `dvc dag` + Graphviz, direct links to all tool UIs

Frontend communicates with backend exclusively via configurable `BACKEND_URL` env var — no shared code.
...
**Alert rules (10):** `HighErrorRate > 5%` (rubric), `DataDriftDetected` (rubric), `ModelNotLoaded`, `HighPredictionLatency`, `LowTestF1`, `LowValF1`, `TrainingDurationHigh`, `IngestFailed`, `FeedbackLoopDead`, `TailLatencySpike`, `LowRequestRate`, `FrequentModelSwitch`

**Grafana:** 17 panels; dashboard auto-provisioned from JSON at startup. Note: Grafana internal port is 3000, host port is 3001 — Docker inter-service config uses `grafana:3000`.
...
- `mlruns/` is bind-mounted into MLflow container and persisted back to the project directory after each CI run
